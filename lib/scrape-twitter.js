...
// ============================================
// FRESHNESS GATE
// A run can hit 100% coverage yet serve a frozen edge-cached timeline: X
// sometimes returns a stale cached profile (or a degraded view to a limited
// account) where every "latest" tweet is days old. The coverage/wipeout checks
// can't see this — they only count *whether* handles produced items, not *how
// old* those items are. computeNewestAgeMs() lets the caller fail the run red
// when the freshest tweet across all handles is older than a sane threshold.
//
// Returns the age in ms of the newest item with a parseable date, or Infinity
// when nothing parses (treated as stale → fail-loud, never silent-green).
// ============================================
function computeNewestAgeMs(items, nowMs) {
  let newestMs = 0;
  for (const item of items || []) {
    const t = Date.parse(item && item.date);
    if (Number.isFinite(t) && t > newestMs) newestMs = t;
  }
  return newestMs ? (nowMs - newestMs) : Infinity;
}
...
async function runTwitterOnly({ sources, countryMap = {}, feedPath = './feed.json', maxItems = 300, shouldTranslate, maxTweetsPerHandle = 5 }) {
  ...
  // Freshness gate: catch "100% coverage but every tweet is days stale" — the
  // frozen-cache failure that coverage checks miss. Threshold is configurable
  // (X_STALE_MAX_HOURS), default 48h: GCC government/news accounts post several
  // times a day, so 48h tolerates weekends/holidays while still catching a
  // multi-day freeze. Only judged when we actually produced items from a
  // meaningful number of handles (else totalWipeout already covers it).
  const staleMaxHours = Number(process.env.X_STALE_MAX_HOURS) || 48;
  const staleMaxMs = staleMaxHours * 60 * 60 * 1000;
  const newestAgeMs = computeNewestAgeMs(items, Date.now());
  const newestAgeHours = Number.isFinite(newestAgeMs) ? (newestAgeMs / 3600000).toFixed(1) : 'n/a';
  const staleWipeout = attempted >= 10 && producing > 0 && newestAgeMs > staleMaxMs;
  if (producing > 0) {
    console.log(`Freshness: newest scraped tweet is ${newestAgeHours}h old (threshold ${staleMaxHours}h)`);
  }
  if (staleWipeout) {
    console.log(`!! WARNING: ${producing}/${attempted} handles produced items, but the NEWEST tweet`);
    console.log(`   is ${newestAgeHours}h old — older than the ${staleMaxHours}h freshness threshold.`);
    console.log(`   X is almost certainly serving a frozen/cached timeline (stale auth cookies,`);
    console.log(`   a locked burner account, or an edge-cache snapshot). Failing the run red so`);
    console.log(`   this can't masquerade as a healthy "success" while the live Wire goes stale.`);
  }
  ...
  return { items, results, added, total, attempted, producing, ratio, totalWipeout, staleWipeout, newestAgeMs };
}
