...
async function initBrowser() {
  ...
  try {
    ...
    // Cloudflare's managed challenge on x.com screens for automation tells.
    // Default headless Playwright advertises them loudly (navigator.webdriver,
    // AutomationControlled), which gets this runner served "Just a moment..."
    // instead of a timeline. Suppress the obvious ones.
...
// x.com sits behind Cloudflare, which serves a "Just a moment..." managed
// challenge to some CI runner IPs. It is NOT a login wall — the old detector
// missed it entirely, so a fully challenged run reported 45x "OK (0 tweets)"
// and looked green while producing nothing. Detect it explicitly.
const CHALLENGE_RE = /Just a moment|Performing security verification|verifies you are not a bot|Checking your browser|Enable JavaScript and cookies to continue/i;

function detectBotChallenge(bodyText, title = '') {
  return CHALLENGE_RE.test(bodyText) || CHALLENGE_RE.test(title);
}
...
async function scrapeOneHandle(source, opts = {}) {
  ...
  let context;
  try {
    // Real-Chrome UA + masked webdriver flag, same narrow viewport the x.com
    // extraction selectors expect (600x900 renders the single-column timeline).
    ...
    // Cloudflare managed challenge: normally auto-clears within ~5-15s once the
    // browser completes the JS check, then the real timeline renders. Wait for
    // it rather than recording a false zero. If it never clears, this runner IP
    // is blocked and the caller aborts the run (see runTwitterOnly).
    let challenged = detectBotChallenge(bodyText, await page.title().catch(() => ''));
    // Distinguish "never challenged" from "challenged then cleared" — otherwise
    // simply drawing a clean runner IP is indistinguishable from a real fix.
    const wasChallenged = challenged;
    if (challenged) {
      try {
        await page.waitForSelector('article[data-testid="tweet"]', { timeout: 15000 });
      } catch (e) { /* challenge did not clear */ }
      bodyText = await page.evaluate(() => document.body?.innerText || '');
      challenged = detectBotChallenge(bodyText, await page.title().catch(() => ''));
    }
    const challengeCleared = wasChallenged && !challenged;
    ...
    // DIAGNOSTIC (temporary — remove once the 0% root cause is known): x.com
    // gave us nothing. Record what it ACTUALLY served before the syndication
    // fallback navigates the page away. Without this, a 0/45 run only tells us
    // that it happened, never why — the "OK (0 tweets)" black box.
    let diag = null;
    if (tweets.length === 0) {
      const counts = await page.evaluate(() => ({
        ...
        anyArticles: document.querySelectorAll('article').length,
      })).catch(() => ({}));
      diag = {
        ...counts,
        wallDetected: wall,
        ...
        sample: bodyText.replace(/\s+/g, ' ').slice(0, 220),
      };
    }
    ...
    return { ...source, tweets, error: null, viaWall: wall, challenged, challengeCleared, diag };
  } catch (e) {
...
async function runTwitterOnly({ sources, countryMap = {}, feedPath = './feed.json', maxItems = 300, shouldTranslate, maxTweetsPerHandle = 5 }) {
  ...
  // DIAGNOSTIC (temporary): confirm the session secrets actually reached the
  // runner. Prints presence and length ONLY — never the cookie values.
  const _at = process.env.X_AUTH_TOKEN || '';
  const _ct = process.env.X_CT0 || '';
  console.log(`X auth: X_AUTH_TOKEN ${_at ? `set (len ${_at.length})` : 'MISSING'}, `
            + `X_CT0 ${_ct ? `set (len ${_ct.length})` : 'MISSING'}`);
  ...
  let abortedOnChallenge = false;
  for (const source of sources) {
    const r = await scrapeOneHandle(source, { shouldTranslate, maxTweetsPerHandle });
    results.push(r);
    const tweetCount = (r.tweets || []).length;
    const tag = r.challenged ? ' [BLOCKED: cloudflare bot challenge]'
              : r.challengeCleared ? ' [cloudflare challenge cleared]'
              : r.viaWall ? ' [via syndication / login wall]'
    ...
    // A persistent challenge is IP-level, not per-handle: if the first handles
    // all stay blocked, the remaining 40+ will too. Abort instead of burning
    // ~25 min of runner time to reach a foregone 0%.
    consecutiveChallenged = r.challenged ? consecutiveChallenged + 1 : 0;
    if (consecutiveChallenged >= 5) {
      abortedOnChallenge = true;
      console.log(`\n!! ABORT: ${consecutiveChallenged} consecutive handles blocked by the `
                + `Cloudflare bot challenge — this runner IP is blocked. Skipping the `
                + `remaining ${sources.length - results.length} handles.`);
      break;
    }

    // DIAGNOSTIC (temporary): dump what x.com served for the first few empty
    // handles, so a 0% run explains itself instead of just reporting 0%.
    if (tweetCount === 0 && r.diag && diagPrinted < 3) {
      diagPrinted++;
      const d = r.diag;
      console.log(`     -> diag url=${d.url} | title="${d.title}" | wallDetected=${d.wallDetected}`);
      console.log(`        tweetArticles=${d.tweetArticles} anyArticles=${d.anyArticles} bodyLen=${d.bodyLen}`);
      console.log(`        body="${d.sample}"`);
    }
  }
  await closeBrowser();
  ...
  const challengedCount = results.filter(r => r.challenged).length;
  const clearedCount = results.filter(r => r.challengeCleared).length;
  if (challengedCount > 0 || clearedCount > 0) {
    console.log(`Cloudflare challenge: ${challengedCount}/${results.length} blocked, `
              + `${clearedCount} cleared after waiting`
              + `${abortedOnChallenge ? ' (run aborted early)' : ''}.`);
  }
  if (attempted >= 10 && ratio < 0.5) {
    console.log(`!! WARNING: less than half of attempted handles produced feed items.`);
    if (challengedCount > 0) {
      console.log(`   Cause: x.com served a Cloudflare "Just a moment..." challenge to this`);
      console.log(`   runner IP. Auth is fine — this is IP reputation, not expired cookies.`);
      console.log(`   Do NOT rotate X_AUTH_TOKEN/X_CT0 for this failure mode.`);
    }
    console.log(`   Other causes: X selector regression, login wall, expired/missing X auth, or rate limiting.`);
    if (!hasAuthSession()) {
